...
项目内字段、规则和章节映射现已有版本草稿、试算、发布入口，详见下节。配置级审批与适用条件尚未实现；报告版本与来源全水位仍需深化；章节目录已支持新增、重命名和相邻移动，拖拽/批量排序与光标精确恢复仍待做；候选可按块采用，人工块虽已保护但没有任意句内自动合并/冲突解决；长时间模型调用与导出尚不是服务重启可恢复的持久任务。这些不应在界面或对外说明中写成已完成。详见 [验收记录](方案推演与写作工作台验收_2026-09-26.md)。
