...
通用章节候选只读取报告选中的运行快照与配置章节，使用其 `result_keys` 生成摘要和两列表格，数值在段落及表格单元格中绑定稳定 `run_id/result_key`。候选仍走预览签名、取消和确认入稿、报告版本和 Plate 往返。通用配置未纳入模型输入契约，因此界面只给确定性内容入口；历史三章继续使用既有受审路径。该配置不含证据门槛、条件句和专门论证规则，不能把该摘要称为自动完成专业报告。验收见[规则与章节配置验收](规则与章节配置验收_2026-09-26.md)。
