...
`App.tsx` 对所有项目使用同一份主导航配置：项目文件、项目资料、项目事实、规则计算、报告写作。语料项目的历史资料仍受服务端只读约束；项目文件、事实与规则组件对该项目隐藏新增或修改入口。普通项目的项目资料页在 `has_corpus=false` 时直接显示空状态，不请求仅限历史语料的接口。全局模型配置不随项目切换改变。
