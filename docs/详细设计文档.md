...
- `POST /api/projects/{project_id}/reports/{report_id}/sections/change`：`action=add|rename|move`、`base_version`、可选标题/标题块 ID/移动方向；按项目归属与报告版本加锁核对，整章保存为新报告版本。空标题、重名、越界移动与旧版本直接拒绝。
- `GET /api/projects/{project_id}/reports/{report_id}/export?level=...`：通过核对闸门后按报告版本、交付级别和审计水位复用或生成不可变 ZIP；`GET .../exports` 只列元数据，`GET .../exports/{export_id}` 校验 SHA256 后返回原始字节。全部接口先核对项目与报告归属。
...
章节目录从已保存报告的 H2 块派生，标题块 ID 是操作目标。`report_sections.change_section` 将首个 H2 之前的标题/引言作为前缀，后续以每个 H2 到下个 H2 之前的所有块作为一章；上移和下移交换整个组，保留段落、表格、`section_id`、`analysis_refs` 和 `source_refs`。重命名仅改变 H2 文本并标记人工标题，后续章节候选更新不会覆盖它。新增章使用新 `manual-*` 章节 ID 与空白段落，不复制历史值或已有章节配置。每次改变先校验内容及来源、推演引用，再保存不可变 `report_versions` 并清除当前核对状态；旧报告版本和导出顺序依旧可核对。前端在正文有未保存改动时阻止目录操作；选择无适用配置的手写章时生成候选保持禁用。详见[章节目录管理验收](章节目录管理验收_2026-09-26.md)。

`report_exports` 保存成功交付的 ZIP 完整字节及 `project_id/report_id/report_version/level/analysis_run_id/watermark_sha256/archive_sha256`。导出在锁定报告的事务内再次执行现有闸门，以当前审计 JSON 的规范化摘要作为水位；同版本、级别、水位重复请求复用已存字节。新文件在审计中记录 `export_id/exported_at/export_level/export_watermark_sha256`，下载旧文件只读取保存的字节，不按当前事实或规则重新生成。失败导出不写记录；列表读取元数据列，避免加载全部 ZIP。当前 PostgreSQL 保存 ZIP 适合本机限定容量；大体量报告需另做配额、归档和异步任务。

Plate 正文每次变更向本浏览器 `sessionStorage` 写项目/报告隔离的待保存 AST 与基础报告版本；服务端保存成功且没有新编辑时删除副本，保存期间又有新编辑时将副本基础版本更新到刚保存的版本。刷新读取服务器正文后，仅在两者不同才弹恢复选择。版本相同或不同都须用户主动选择恢复；版本不同会说明恢复后保存产生新版本，原服务器内容留在 `report_versions`。恢复后的正文仍走现有预览、校验与版本检查，放弃只删除会话副本。侧栏关闭通过 Plate 编辑器公开的 `focus/focusBlock` 操作恢复焦点与选择；章节定位同时将光标移至对应标题。此机制不提供跨浏览器会话同步，也不自动合并同时修改的同一段落。验证见[交付与编辑恢复验收](交付与编辑恢复验收_2026-09-26.md)。
...
局部更新在此基础上对引导块逐项生成操作。段落数字、表格单元格及顶层 `analysis_refs` 同步指向新 `run_id`；S4 的已知需求条件句是独立操作，更新数字后若仍保留旧判断，`ANALYSIS_CONDITION_STALE` 阻断核对。S7.1 从当前供应商输入重新构造受审判断，缺少报价时保持待补。人工编辑块有专门的 `manual_review` 操作：只在旧数字与绑定结果逐一定位、替换目标不歧义，或条件句与版本配置精确匹配时提供前后全文对照；用户勾选后才原子更新，保留块 ID、未关联文字、格式和表格结构。重复旧数值位置、非数值供应商引用等无法安全定位时继续保持不可选择和导出阻断。每次提交写入报告版本、操作事件和章节刷新基线；导出审计增加 `analysis_refreshes`，记录所选操作和运行。再次预览只呈现剩余操作。检查面板合并同一段落的重复旧引用提示，原始逐引用审计仍保留。该范围不涵盖任意自由文本的语义判断或复杂拆分合并；验证见[局部更新验收](局部更新验收_2026-09-26.md)及[人工编辑局部更新验收](人工编辑局部更新验收_2026-09-26.md)。

人工块无法安全自动替换时，检查面板提供“编辑正文”定位。用户先在 Plate 改写并保存，`refresh/preview` 再从**已保存报告**和所选不可变运行计算可选动作：`rebind` 只改引用，不改文字/格式/块 ID；逐段或逐表格单元格核对本段已绑定结果键的新值、单位、出现次数及已配置条件的方向，旧值仍可见时拒绝。`detach` 要求旧引用值和已知条件句已从正文移除，然后只删除该块及其单元格的 `analysis_refs`。数值核对识别千分位写法，避免仅换成 `300,000` 即绕过旧值检查。预览呈现每个结果键的旧值→新值或解除结果，同一人工块不能同时提交重绑和解除；提交时重新计算、核对版本与项目归属，写入新报告版本和 `apply_refresh` 事件。取消不写业务对象；解除引用后仍需通过已有事实、来源和数字闸门，不能因解绑直接获得交付状态。

澄岳的历史原文来源与当前方案计算值可能并存：人工改正文并保存时，历史语料数字校验仅额外放行**该块已绑定结果键在报告当前运行中的值**，不是放行当前运行全部数字，也不改变历史来源或把新值冒称原文。旧运行引用在重绑前仍触发 `ANALYSIS_RUN_STALE`，无法通过核对/导出。当前算法不解释任意自由文字的因果或身份含义；含糊表述必须人工核对，复杂 Plate 块拆合后的引用迁移仍未实现。测试见[人工引用重绑与解除验收](人工引用重绑与解除验收_2026-09-26.md)。

### 本轮未实现的设计目标

项目内字段、规则和章节映射现已有版本草稿、试算、发布入口，详见下节。配置级审批与适用条件尚未实现；报告版本与来源全水位仍需深化；章节目录已支持新增、重命名和相邻移动，拖拽/批量排序仍待做；侧栏关闭与章节定位已恢复编辑焦点，跨复杂弹层的全部选择状态仍待验证；候选可按块采用，人工块虽已保护但没有任意句内自动合并/冲突解决；模型调用与长时间导出尚不是服务重启可恢复的持久任务。成功的同步导出已持久保存和可重下载，不等同于后台任务持久化。详见 [验收记录](方案推演与写作工作台验收_2026-09-26.md)。
...
新运行后的 `refresh/preview` 将未改写的配置条件句作为独立 `config_condition` 动作，按本次快照的结果更新文字及依赖引用；已改写的条件句和旧模型段落需要逐处核对，安全可定位时可采用 `manual_review`，否则继续阻断。报告核对检查识别独立或嵌在句内的反向条件句，以及事实/运行不匹配。模型采用事件记录实际采用的模型块 ID、模型调用元数据和运行 ID；导出审计并列保留历史模型事件与人工局部更新事件，密钥不进入审计。DOCX/PDF 使用已核对的当前报告版本与运行生成情景分析文件，前次导出文件保持不变。完整实测见[非可研验收](非可研条件证据写作验收_2026-09-26.md)及[人工编辑局部更新验收](人工编辑局部更新验收_2026-09-26.md)。

## 2026-09-26 增补：统一项目导航与环评报告验证（NEXT-24）

`App.tsx` 对所有项目使用同一份主导航配置：项目文件、项目资料、项目事实、规则计算、报告写作。语料项目的历史资料仍受服务端只读约束；项目文件、事实与规则组件对该项目隐藏新增或修改入口。普通项目的“项目资料”由 `ProjectMaterialsView` 汇总本项目 `documents` 和 `facts` 接口的真实记录，显示原件、解析段落数量及事实；点事实调用项目归属检查过的 `/facts/{key}/source` 回链原文或规则，点原件通过 URL 中的 `document` 参数进入“项目文件”并支持刷新恢复。普通项目不调用历史 `corpus` 接口，也不自动构造 30 类空产物。全局模型配置不随项目切换改变。

页面 URL 记录 `project`、`section`、可选 `report`；从报告跳转其他项目页面时保留报告 ID，`section` 决定刷新时显示的页面，返回报告可恢复同一文章；浏览器 `popstate` 同步项目与页面。切换项目时清除旧报告和页面参数，防止跨项目误开。侧栏离开前派发可取消的导航事件；工作台在正文自动保存未完成、方案输入未保存时阻止切换，旧报告编辑器和配置草稿也在有未保存改动时阻止切换。该恢复逻辑只处理用户可见路由状态，不能替代服务端项目归属检查。

第二类真实报告使用既有版本化配置、项目事实证据、Decimal规则、运行快照、候选与Plate，没有在代码里加入环评专属常量。江门公开环评第5页两项投资数据由独立项目原件位置 `p5-s8` 支撑；预算上限是用户方案假设，`max(budget_limit-environmental_investment,0)` 与双向条件句冻结在运行中。20万元与15万元两次运行为差额3万元与有效0万元；模型只在含条件和原件证据的第二章起草，确定性第一章不冒称模型。新运行时，第一章数值虽未变化，仍须通过 `refresh/commit` 重绑到本次运行；第二章重新起草后两章均重新核对。完整实测与边界见[环评报告写作验收](环评报告写作验收_2026-09-26.md)。

## 2026-09-27 增补：长文档定位与方案事实水位（NEXT-25）

`GET /api/projects/{project_id}/documents/{document_id}/search?q=&offset=&limit=` 在已持久化的片段中大小写折叠匹配，返回总数、页码、位置和有限摘要；先按项目归属读取原件，不返回其他项目的内容。当前实现按片段线性扫描，`limit` 最大 50；属于本机中等文档可用实现，大规模资料需要数据库文本索引、超时和压力基准。前端在 URL 保存 `document/page/segment`，点击命中项高亮并滚动至片段；切换项目或文件时清理旧定位。全文请求有代次标识，迟到结果不能回填到新文件。上传在浏览器先检查扩展名、空文件和 30 MB 上限，服务端仍独立校验。

新运行的 JSON 快照增加 `project_fact_baseline`：对运行创建时与输入值、单位吻合的同项目事实，记录修订、值和单位；旧运行不被改写。章节证据在当前事实与运行不同且当时基线含该字段时，认定为事实事后变化并阻断；运行创建时就不吻合、输入明确为 `scenario_assumption` 时，认定为有披露要求的方案假设。旧快照没有该字段，沿用保守阻断。`ASSUMPTION` 在确定性候选与报告检查中是备注，模型候选仍要求 `VERIFIED`，正式事实稿仍被方案假设闸门阻断。此字段随既有不可变运行快照落库，不改表结构；计算摘要覆盖它。

`GET /reports` 使用现有检查结果形成工作稿、待核对、已核对三态；已核对需要报告内容哈希与当前版本吻合且无阻断项，披露性备注不降级。额外返回 `has_scenario_assumption`，列表和正文标题显示“方案假设”；不能让用户把已人工核对误读成事实已被证实。配置章节的确定性候选只写数据与受控条件，不再把随流程变化的“结论待核对”写进永久正文。事实列表增加当前修订的原文位置核对状态，前端只在需要时展开证据。报告标题筛选、事实状态筛选和文件名筛选在本地已加载列表执行，不改变服务端权限边界。详情抽屉使用 dialog 语义和 Esc 关闭。测试库的真实链路与受控浏览器交互分别记录，不混称为模型或专业语义验证。

## 2026-09-27 增补：可研章节预览、模型校验与推演依据（NEXT-26）

配置章节的模型路径仍由 `analysis_writing._configured_model` 组装筛选后的本章值、原件证据审核状态、已批准条件句和禁用语。服务端分别校验 JSON 结构及 `used_result_keys`、来源外数字、禁用语和条件句；一次响应不合格时追加短反馈再请求一次，最终失败返回 422，未调用候选确认事务。成功调用的 `model_call.validation_attempts` 进入候选及后续采用审计。重试不会放宽校验、补造结果引用或改写报告。模型不可达仍返回 503。

前端候选面板按 AST 类型预览段落和两列表格，显示“计算内容 / 结果表 / 模型候选”，逐块勾选；依据折叠项从冻结的运行 `results` 显示业务名称与单位，点击转至对应计算详情。候选证据全通过时使用中性成功状态，缺证据仍显示原因。报告顶部显示当前方案与运行修订。桌面打开面板时，章节导航排成可滚动横条，编辑器与面板并列；窄屏保持抽屉。仅变更展示与布局，不复制候选到项目事实。

`report_pipeline._analysis_basis` 从导出审计中的当前 `analysis_run`、正文引用和运行轨迹，沿依赖向上收集实际引用结果所需的输入。人类可读的 DOCX/PDF 在正文后附“推演依据”：输入值及 `origin` 映射的来源类别，计算结果、中文字段名公式和代入值。未被正文引用的指标不进入附录；完整快照仍在 `audit.json`。PDF 的该节使用紧凑段落样式，避免北京固定两章样本出现单行孤页。导出依旧绑定不可变报告版本与运行，不因新的附录改变事实核对或情景分析闸门。

导出审计增加 `render_version` 并纳入水位摘要。现有交付表仍按报告版本、交付级别与水位复用；格式升级会产生新的水位和交付记录，老 ZIP 原样留存并按原 ID 重下载。同一格式重复请求仍返回同一交付 ID。此字段不触发数据库迁移。
