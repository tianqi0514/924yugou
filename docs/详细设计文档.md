...
当前版本适合本机试点和受控演示。核心路径为：项目文件 → 待审候选 → 项目事实 → 确定性规则 → 章节候选 → Plate 工作稿 → 人工核对 → DOCX/PDF/审计包。历史澄岳精密语料作为单独项目中的只读资产，新项目经明确选择后只读引用其结构和模式。
...
| 模型 | 抽取、章节起草、OCR 的任务路由和配置已接入 | 模型章节模式实际仅接收已筛选的本项目事实；尚未发送完整、带版本来源的写作包 |
...
所有项目业务请求从路径中的 `project_id` 进入。文档、报告、候选、事实及证据绑定均检查项目归属。历史语料项目的写接口拒绝修改。`project_corpora` 表将只读语料绑定到历史项目；普通项目不因此获得任意历史项目读权。
...
| `writing_commit_events` | UUID | 确认写入的报告/版本/章节、语料版本、模式、块 ID、来源、问题、已确认条目 |
...
引导模式生成明确标识的确定性候选。模型模式仅开放 S4/S7.1，当前实现调用 `model_section(title, selected_project_facts)`，再做数字、事实 key、覆盖、特定历史残留与条件句检查，并由服务端挂上来源。**它尚未把结构、规则、论断、证据及其版本元数据作为完整写作包传给模型**；后续应补受控输入协议与实际消费记录，不能把服务端附带回链等同于模型已读取这些资料。

模型未配置或调用失败返回不可用，不用确定性文字冒充模型结果。固定字符串检查只覆盖本次已知风险，不是通用语义审稿器。
...
“去掉类别后固定构造器报错”只说明实现耦合，不能证明用户必须保留该独立文件形式；等价替换与更多真实章节测试仍缺。逐类证据与遮蔽边界以 [写作支撑验证报告](写作支撑验证报告.md) 为准。30 类历史资产全部保留，后续根据信息契约决定新项目按需生成哪些业务数据，而非强制创建 30 份空文件。
...
普通项目主导航为项目文件、项目事实、规则计算、报告写作；案例验证与模型配置作为次要入口。澄岳历史项目显示资料浏览入口，不能出现可编辑历史事实的假操作。

报告页按起草、编辑、核对导出组织操作。主要展示选择项、数值、来源、问题与结果；长说明放文档/详情，不在页面堆积说明性段落。状态与错误反馈仍须保留，否则用户无法知道动作是否完成。
...
| [frontend/tests](../frontend/tests) | 壳层、事实源/影响、章节映射与候选、编辑交互的浏览器测试 |
| [UI 体验优化与验收](UI体验优化与验收_2026-09-24.md) | 真实前后端闭环、保存刷新、表格/事实 token、事实变更与导出、窄屏 |
| [写作支撑验证报告](写作支撑验证报告.md) | 三种包、21 组案例、13 次遮蔽、三节预审稿与实验限制 |

截至该验收记录：后端 **55 passed**、前端 Playwright **9 passed**、前端构建通过。此处引用已有实际运行记录，不将本次文档更新表述成重新运行所有测试。前端自动化中有 API 替身测试；真实数据库/浏览器人工闭环单独记录，不能混称所有浏览器用例均为真实后端端到端测试。
...
| 增量 | 设计方向 | 验收重点 |
| --- | --- | --- |
| 受控写作包 V2 | 为选中条目生成带用途、版本、来源的最小模型输入；记录实际发送和消费类别 | 模型无法看到未审核历史值；可检查输入摘要与输出来源；失败不写报告 |
...
当前最重要的技术欠账是：让已审核章节包真正成为可审计的模型输入；增加独立数据与等价替换试验；补齐候选/导出持久化审计和版本发布边界。向量检索、图片生成、独立图库和实时协作只有在实际用户任务证明需要时再接入。
