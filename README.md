...
本轮已用每章资料配置和只读模拟观察了 S4、S7.1、S5.2 的 90 次类别移除，并完成 S4/S7.1 的真实模型起草、取消和确认。下一步是 **NEXT-02 等价信息替换实验 → NEXT-03 审计补全**；单次样本的成功不能推断其他章节或报告类型的质量。不为凑够30类而制造调用，也不在本轮重新启动已暂缓的抽取扩展。
...
在普通项目的“报告写作”选择只读参考和章节后，可展开“资料应用”。七组中的每一类都有“作用”入口，并可将本章使用方式设为“自动选用 / 仅核对 / 不使用”。保存的是本项目本章节的配置，保存后写作包刷新，先前的条目审核与候选须重新完成。一次可勾选1–6类做移除模拟，比较原配置、当前设置与逐类移除的候选差异；可用本项目事实或仅用于模拟的示例值。模拟不会写入事实、规则或报告。当前仅 S4、S7.1、S5.2 支持候选对比，其余章节可查看作用和保存配置，但不据此判断写作贡献；“移除后受阻”首先表示当前实现有依赖，不证明该类信息不可替代。
...
Playwright自动启用5174端口，浏览器测试使用受控API响应，不要求生产后端。它们验证交互，不等同真实数据库端到端；真实前后端联动另记在本轮验收报告，自动化补齐列为NEXT-13。

2026-09-24 本轮后端全量运行 81 项，结果为 **80 通过、1 失败**；失败来自 CAT-13 测试夹具把目录误设成仅有 R006，已修正预期并将相关测试文件复跑为 **11/11 通过**，尚未把这两次运行合称“全量 81/81”。前端全量 Playwright **11/11 通过**，构建通过；本模块模型输入及写作包定向回归另复跑 **3/3 通过**。应用数据库升级前后核对为 15 项目、24 事实、11 报告、3,462 语料记录和 117 资产，数量保持不变。独立测试库完成三章×30类共90次只读移除与5个固定输入案例，前后项目/事实/报告/配置快照不变；S4/S7.1各两次真实文本模型请求，首次取消、再次确认，留下2个模型审计。解包检查 DOCX、PDF 与审计中的数值、供应商和预审标记一致；因缺本项目原件，正式导出仍受阻。详情见[本轮配置与推演验收](docs/30类资料配置与推演验收.md)。这些工作稿仍需专业人员核对，不能视为整份专业报告自动完成。
