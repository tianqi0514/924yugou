...
5. Plate 正文可编辑、输入 `/` 调出菜单，工作稿自动保存，Cmd/Ctrl+S 可手动保存。修改方案后重新选择运行，核对影响并重生成相应章节；改过的块会保留，旧运行引用仍须手工处理才能通过闸门。最后在“检查与导出”处理阻断项、人工核对，下载“情景分析报告”ZIP。正式事实结论导出会阻断历史假设。
...
本轮新写作路径直接消费部分 CAT-09/12/13/15/18 记录；30 类资产仍全部入库可浏览，历史资料的逐类配置与移除实验在次级“资料起草”入口可用。**30 类可浏览不等于 30 类都支撑了当前成稿。** 章节排序、逐段候选、人工改写的局部合并、规则独立修订和持久任务恢复仍在[开发清单](docs/开发清单.md)中。
