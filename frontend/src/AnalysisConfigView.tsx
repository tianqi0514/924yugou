...
type ConfigSection = { id: string; title: string; result_keys: string[]; evidence_keys?: string[]; conditions?: ConfigCondition[]; forbidden_terms?: string[] }
...
const emptySection = (): ConfigSection => ({ id: '', title: '', result_keys: [], evidence_keys: [], conditions: [], forbidden_terms: [] })
const emptyCondition = (): ConfigCondition => ({ id: '', expression: '', when_true: '', when_false: '' })
  ...
  return <main className="page analysis-config-page">
    ...
    <div className="analysis-config-layout"><aside className="analysis-config-list">{configs.map((item) => <button key={item.id} className={selectedId === item.id ? 'active' : ''} onClick={() => choose(item)}><strong>v{item.version} · {item.name}</strong><small>{item.status === 'DRAFT' ? '草稿' : '已发布'}</small></button>)}{!configs.length && <div className="empty">暂无配置。点击“新建版本”开始。</div>}</aside>
      {editing && <section className="analysis-config-editor"><div className="analysis-config-editor-head"><input aria-label="当前配置名称" value={editing.name} disabled={!writable} onChange={(event) => edit({ ...editing, name: event.target.value })} /><div>{writable && <><button disabled={!changed || busy} onClick={() => { setEditing(selected); setTested(null) }}>放弃修改</button><button disabled={!changed || busy} onClick={() => void save()}><Save size={13} /> 保存草稿</button></>}</div></div>
        ...
        {tab === 'sections' && <div className="analysis-config-rows">{editing.sections.map((section, index) => <div className="analysis-config-section" key={index}>
          <div className="analysis-config-section-head"><input aria-label={`章节 ${index + 1} 标识`} placeholder="章节 ID" value={section.id} disabled={!writable} onChange={(event) => patchSection(index, { ...section, id: event.target.value })} /><input aria-label={`章节 ${index + 1} 标题`} placeholder="章节标题" value={section.title} disabled={!writable} onChange={(event) => patchSection(index, { ...section, title: event.target.value })} />{writable && <button aria-label={`删除章节 ${index + 1}`} onClick={() => edit({ ...editing, sections: editing.sections.filter((_, at) => at !== index) })}><Trash2 size={14} /></button>}</div>
          <div className="analysis-config-section-label">展示指标</div><div className="analysis-config-key-list">{numericFields.map((field) => <label key={field.key}><input type="checkbox" disabled={!writable} checked={section.result_keys.includes(field.key)} onChange={(event) => patchSection(index, { ...section, result_keys: event.target.checked ? [...section.result_keys, field.key] : section.result_keys.filter((key) => key !== field.key) })} />{field.label} <small>{field.key}</small></label>)}</div>
          <div className="analysis-config-section-label">原件证据必需</div><div className="analysis-config-key-list">{inputs.map((field) => <label key={field.key}><input type="checkbox" aria-label={`证据 ${field.label}`} disabled={!writable} checked={(section.evidence_keys || []).includes(field.key)} onChange={(event) => patchSection(index, { ...section, evidence_keys: event.target.checked ? [...(section.evidence_keys || []), field.key] : (section.evidence_keys || []).filter((key) => key !== field.key) })} />{field.label} <small>{field.key}</small></label>)}</div>
          <div className="analysis-config-section-label">条件判断</div>{(section.conditions || []).map((condition, at) => <div className="analysis-config-condition" key={at}><input aria-label={`章节 ${index + 1} 条件 ${at + 1} 标识`} placeholder="条件 ID" value={condition.id} disabled={!writable} onChange={(event) => patchSection(index, { ...section, conditions: (section.conditions || []).map((row, position) => position === at ? { ...row, id: event.target.value } : row) })} /><input aria-label={`章节 ${index + 1} 条件 ${at + 1} 表达式`} placeholder="如 demand > capacity" value={condition.expression} disabled={!writable} onChange={(event) => patchSection(index, { ...section, conditions: (section.conditions || []).map((row, position) => position === at ? { ...row, expression: event.target.value } : row) })} /><input aria-label={`章节 ${index + 1} 条件 ${at + 1} 成立`} placeholder="成立时文字" value={condition.when_true} disabled={!writable} onChange={(event) => patchSection(index, { ...section, conditions: (section.conditions || []).map((row, position) => position === at ? { ...row, when_true: event.target.value } : row) })} /><input aria-label={`章节 ${index + 1} 条件 ${at + 1} 不成立`} placeholder="不成立时文字" value={condition.when_false} disabled={!writable} onChange={(event) => patchSection(index, { ...section, conditions: (section.conditions || []).map((row, position) => position === at ? { ...row, when_false: event.target.value } : row) })} />{writable && <button aria-label={`删除章节 ${index + 1} 条件 ${at + 1}`} onClick={() => patchSection(index, { ...section, conditions: (section.conditions || []).filter((_, position) => position !== at) })}><Trash2 size={14} /></button>}</div>)}{writable && <button className="analysis-config-add" onClick={() => patchSection(index, { ...section, conditions: [...(section.conditions || []), emptyCondition()] })}><Plus size={13} /> 添加条件</button>}
          <div className="analysis-config-section-label">模型禁用表述</div><input aria-label={`章节 ${index + 1} 禁用表述`} placeholder="用逗号分隔" value={(section.forbidden_terms || []).join('，')} disabled={!writable} onChange={(event) => patchSection(index, { ...section, forbidden_terms: event.target.value.split(/[，,]/).map((item) => item.trim()).filter(Boolean) })} />
        </div>)}{writable && <button className="analysis-config-add" onClick={() => edit({ ...editing, sections: [...editing.sections, emptySection()] })}><Plus size={13} /> 添加章节</button>}</div>}
