...
function CandidateCard({ item, segments, projectId, documentId, reload, expanded, onToggle }: { item: Candidate; segments: Segment[]; projectId: string; documentId: string; reload: () => Promise<void>; expanded: boolean; onToggle: () => void }) {
  ...
  return <div className={`candidate-card candidate-compact ${expanded ? 'expanded' : ''}`}>
...
export default function DocumentsView({ project, notify }: { project: Project; notify: (message: string) => void }) {
  ...
  const loadList = useCallback(async () => {
    ...
    setDocuments(items)
    setDocumentId((old) => items.some((item) => item.id === old) ? old : items[0]?.id || '')
  }, [project.id])
  ...
  const navigate = (target: number) => {
    ...
    setError(''); setPage(target); setRangeValue(String(target))
  }
  const upload = async (file: File | undefined) => {
    ...
    try {
      ...
      await loadList(); setDocumentId(created.id); setPage(1); setRangeValue('1'); setBatchResults([]); setBatchProgress({ done: 0, total: 0, current: 0 }); notify('原件已保存')
    } catch (cause) { setError((cause as Error).message) } finally { setBusy(false) }
  }
  ...
  return <main className="page"><div className="breadcrumb">项目 / {project.name} / 项目文件</div><div className="page-header"><div><h1>项目文件</h1></div><label className="primary-button upload-button"><FileUp size={15} /> 上传 PDF / DOCX<input type="file" accept=".pdf,.docx" disabled={busy || batchBusy} onChange={(event) => { void upload(event.target.files?.[0]); event.target.value = '' }} /></label></div>
    {error && <div className="notice error">{error}</div>}
    <div className="document-layout"><aside className="workspace-card document-list"><h2>已上传文件</h2>{documents.length === 0 ? <p className="empty">暂无文件</p> : documents.map((item) => <button className={item.id === documentId ? 'active' : ''} key={item.id} disabled={busy || batchBusy} onClick={() => { setDocumentId(item.id); setPage(1); setRangeValue('1'); setPageMap([]); setBatchResults([]); setBatchProgress({ done: 0, total: 0, current: 0 }); setError('') }}><strong>{item.filename}</strong><small>{item.file_kind.toUpperCase()} · {item.pages} {item.file_kind === 'pdf' ? '页' : '份'} · {item.status === 'OCR_REQUIRED' ? '部分页需 OCR' : '可查看'}</small></button>)}</aside>
      <div className="document-main">{detail ? <><section className="workspace-card"><div className="workspace-toolbar"><div><h2>{detail.document.filename}</h2></div><div className="toolbar"><a className="subtle-button" href={`/api/projects/${project.id}/documents/${documentId}/original`} target="_blank" rel="noreferrer">打开原件</a><button className="subtle-button" onClick={() => void loadDetail()}><RefreshCw size={13} /> 刷新</button></div></div><div className="document-page-nav"><button disabled={page <= 1 || batchBusy} onClick={() => navigate(page - 1)}>上一页</button><span>{detail.document.file_kind === 'pdf' ? `PDF 第 ${page} / ${detail.document.pages} 页` : 'DOCX 文字与表格'}</span><button disabled={page >= detail.document.pages || batchBusy} onClick={() => navigate(page + 1)}>下一页</button><label>跳转 <input aria-label="跳转页码" type="number" min={1} max={detail.document.pages} value={jumpValue} onChange={(event) => setJumpValue(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') navigate(Number(jumpValue)) }} /></label><button disabled={batchBusy} onClick={() => navigate(Number(jumpValue))}>前往</button></div>{pageMap.find((item) => item.page === page) && <div className="document-page-status">{pageMap.find((item) => item.page === page)?.segment_count} 段原文 · {pageMap.find((item) => item.page === page)?.pending_count} 条待审</div>}{pageMap.some((item) => item.pending_count) && <div className="document-pending-pages"><strong>待审页</strong>{pageMap.filter((item) => item.pending_count).slice(0, 24).map((item) => <button key={item.page} className={page === item.page ? 'active' : ''} disabled={batchBusy} onClick={() => navigate(item.page)}>{item.page} <small>{item.pending_count}</small></button>)}{pageMap.filter((item) => item.pending_count).length > 24 && <span>更多待审页</span>}</div>}<div className="document-source-scroll">{detail.segments.length ? detail.segments.map((segment) => <div id={`segment-${segment.ref}`} className={`document-segment ${segment.kind === 'table_row' ? 'document-table-row' : ''}`} key={segment.ref}><small>{segment.ref} {segment.locator || ''}{segment.kind === 'table_row' ? ' · 表格行' : segment.kind === 'ocr' ? ' · OCR 待核对' : ''}</small><p>{segment.text}</p></div>) : <div className="empty">本页无可读取文字。{detail.document.file_kind === 'pdf' && <div className="inline-actions"><button className="primary-button" disabled={busy || batchBusy || !ocrReady} onClick={() => void recognize()}>{busy ? '识别中…' : '识别当前页'}</button>{!ocrReady && <small>扫描页识别未配置</small>}</div>}</div>}</div></section>
        <section className="workspace-card"><div className="workspace-toolbar"><h2>抽取事实</h2></div><div className="document-batch-controls"><input aria-label="待抽取页码" value={rangeValue} onChange={(event) => setRangeValue(event.target.value)} placeholder="如 5,8-10" disabled={batchBusy || busy} /><button className="primary-button" disabled={batchBusy || busy || !modelReady || !pageMap.length} onClick={() => void extractSelected()}><FileSearch size={14} /> {batchBusy ? '抽取中…' : '抽取'}</button>{batchBusy && <button className="subtle-button" onClick={() => { cancelBatch.current = true }}>当前页完成后停止</button>}</div>{batchProgress.total > 0 && <div className="document-batch-progress">已处理 {batchProgress.done}/{batchProgress.total} 页{batchBusy && ` · 正在处理第 ${batchProgress.current} 页`}</div>}{batchResults.length > 0 && <div className="document-batch-results">{batchResults.map((row) => <button key={row.page} type="button" onClick={() => navigate(row.page)} disabled={batchBusy}><b>第 {row.page} 页</b><span>{row.error ? `失败：${row.error}` : row.skipped ? `跳过：${row.skipped}` : `新增 ${row.created} 条${row.refreshed ? `，更新 ${row.refreshed} 条来源校验` : ''}`}</span></button>)}</div>}</section><section className="workspace-card"><div className="workspace-toolbar"><h2>本页候选</h2></div>{!modelReady && <div className="notice warn">事实抽取模型未配置</div>}{detail.candidates.length ? <><div className="candidate-counts">{detail.candidates.filter((candidate) => candidate.review_status === 'PENDING').length} 条待审 · {detail.candidates.filter((candidate) => candidate.review_status === 'PENDING' && !candidate.source_valid).length} 条引用需核对</div>{detail.candidates.map((candidate) => <CandidateCard key={candidate.id} item={candidate} segments={detail.segments} projectId={project.id} documentId={documentId} reload={reloadCurrent} expanded={activeCandidateId === candidate.id} onToggle={() => setActiveCandidateId((old) => old === candidate.id ? null : candidate.id)} />)}</> : <div className="empty">暂无候选</div>}</section></> : <div className="workspace-card empty">选择或上传文件</div>}</div></div>
  </main>
}
