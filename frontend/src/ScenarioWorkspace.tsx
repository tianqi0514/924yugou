...
type Preview = { run_id: string; section_id: string; mode: string; base_version: number; content: Block[]; model_audit: Record<string, unknown>; expires_at: number; preview_token: string; issues: Issue[]; preserved_blocks?: Block[] }
...
export default function ScenarioWorkspace({ project, notify, onOpenFacts, onOpenCorpus, onLegacy }: {
  ...
  const loadLists = useCallback(async () => {
    const [reportRows, scenarioRows, factData] = await Promise.all([
      api<ReportSummary[]>(`${base}/reports`), api<Scenario[]>(`${analysis}/scenarios`),
      api<{ facts: Fact[] }>(`${base}/facts`),
    ])
    setReports(reportRows); setScenarios(scenarioRows); setFacts(factData.facts)
  ...
  useEffect(() => {
    const onPop = () => setReportId(new URLSearchParams(location.search).get('report'))
  ...
  const openReport = (id: string | null) => {
    ...
    setReportId(id); setPanel(null); setCandidate(null); setError('')
  }
  ...
  const createScenario = async (source: 'historical' | 'project' | 'copy') => {
    ...
    try {
      const created = await post<Scenario>(`${analysis}/scenarios`, {
        ...
        source, ...(source === 'copy' ? { copy_from: scenarioId } : {}),
      })
  ...
  const confirmRun = async () => {
    ...
    try {
      ...
      setImpact(null); setRunId(active); await loadReport(report.id)
  ...
  const previewDraft = async () => {
    ...
    try {
      const draft = await post<Preview>(`${analysis}/reports/${report.id}/draft/preview`, {
        run_id: report.analysis_run_id, section_id: selectedSection, mode: draftMode,
      })
      setCandidate(draft)
    } catch (cause) { setError((cause as Error).message) } finally { setBusy(false) }
  }
  ...
  const acceptDraft = async () => {
    ...
    try {
      await post(`${analysis}/reports/${report.id}/draft/commit`, {
        ...candidate, replace_section: report.content.some((block) => block.section_id === selectedSection),
      })
      setCandidate(null); await loadReport(report.id); await loadLists(); setPanel(null)
      notify('章节已加入报告')
    } catch (cause) { setError((cause as Error).message) } finally { setBusy(false) }
  }
  ...
  const resultList = currentRun ? ['N017', 'N034', 'N035', 'first_year_demand', 'qualified_capacity', 'planned_sales', 'demand_gap'].map((key) => currentRun.snapshot.results[key]).filter((row): row is Result => !!row)
    : []
  ...
  return <main className={`page scenario-page ${report ? 'scenario-editor-page' : ''}`}>
    ...
    {!report ? <>
      <div className="page-header"><h1>报告</h1><div className="scenario-inline"><button onClick={onLegacy}>{project.has_corpus ? '历史文章整理' : '资料起草'}</button><button className="primary-button" onClick={() => setCreating(true)}><Plus size={15} /> 新建报告</button></div></div>
      ...
      <div className={`scenario-workspace ${panel ? 'with-panel' : ''}`}><aside className="scenario-outline"><div className="scenario-outline-header">章节</div>{reportHeadings.map(({ block, index }) => <button key={block.id || index} className={selectedPosition === index + 1 ? 'active' : ''} onClick={() => { setSelectedPosition(index + 1); document.querySelectorAll('.plate-content [data-block-id]')[index]?.scrollIntoView({ block: 'center', behavior: 'smooth' }) }}>{textOf(block) || '未命名章节'}</button>)}<button className="scenario-outline-add" onClick={() => setPanel('draft')}><Plus size={13} /> 添加章节</button></aside>
        ...
        {panel && <aside className="scenario-panel" role="complementary"><div className="scenario-panel-head"><h2>{({ inputs: '输入数据', results: '推演结果', draft: '生成本章', sources: '资料与来源', check: '检查与导出', versions: '版本', materials: '资料应用' })[panel]}</h2><button onClick={() => setPanel(null)} aria-label="关闭面板"><X size={18} /></button></div><div className="scenario-panel-body">
          ...
          {panel === 'materials' && <><div className="scenario-material-row"><strong>本章资料</strong><button onClick={onOpenCorpus}>查看 30 类资料</button></div>{report.content.filter((block) => block.section_id === selectedSection).flatMap((block) => block.source_refs || []).map((ref, index) => <button key={index} className="scenario-source-row" onClick={() => void source(ref)}>{ref.semantic_id || ref.record_id} · 查看作用与原文</button>)}<div className="scenario-short-note">实际参与本章的记录显示在此；完整历史资料在项目资料中查看。</div></>}
        </div></aside>}
      </div>
    </>}
  </main>
