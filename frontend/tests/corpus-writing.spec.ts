...
test('a project fact binds an actual document segment only after explicit review', async ({ page }) => {
  ...
  await page.goto('/?project=evidence-project&section=facts')
  await page.getByRole('row', { name: /首年需求/ }).getByRole('button', { name: '证据' }).click()
  await expect(page.getByRole('dialog', { name: /核对原文/ }).locator('.notice')).toContainText('来源待核对')
  ...
  await expect(page.getByRole('dialog', { name: /核对原文/ })).toHaveCount(0)
})
